...
- **`fields`** — inspector controls (`text`, `number`, `toggle`, `target`). Each value is saved to the matching `dataAttribute` and forwarded to your store as context. `key` must be an identifier (letters, digits, underscore); `dataAttribute` must be a plain `data-…` attribute — `data-action` and the `data-wp-` namespace are reserved and rejected. A field marked `required: true` shows an inline warning in the inspector while empty (saving is never blocked). A field marked `optional: true` renders default-hidden in the Interactions panel with a per-item reset. A non-empty `default` is seeded into the block when the author selects the action, so it reaches your store's context without the author touching the field.
- **`target` fields** — render a block picker instead of a typed id: the author picks a block on the page and its anchor is created automatically. Constrain candidates with `"targets": { "blocks": ["core/group"], "shape": "dialog" }` — shapes are named predicates (the plugin ships `dialog`); themes can register more from editor JS via the `blockActions.targetShapes` filter.
...
- **`entry`** — the store action a trigger invokes (default `actions.handleClick`), e.g. `"entry": "actions.open"`. With it, authors can pick WHEN your action fires (click, hover/focus, scrolled into view, page load, timer) plus viewport and reduced-motion conditions — no extra code in your store.
- **`triggers`** — restrict the trigger choices, e.g. `"triggers": ["click", "hover"]`.
- **`structural`** — set `true` when your action owns its whole lifecycle (wires its own handlers in `init`); trigger selection is then hidden and your wiring is untouched.
