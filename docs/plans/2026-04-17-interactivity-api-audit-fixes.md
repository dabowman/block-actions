...
- [ ] **Task 1.1: Wrap all `preventDefault` / `stopPropagation` actions in `withSyncEvent`** 🔴
...
- [ ] **Task 1.2: Wrap carousel touch/observer callbacks in `withScope`** 🔴
...
- [ ] **Task 1.3: Remove or localize `window.blockActions.nonce` / `.debug`** 🔴
...
- [ ] **Task 2.1: Rewrite `toggle-visibility` as declarative directives** 🟠
...
- [ ] **Task 2.2: Rewrite `copy-to-clipboard` feedback to declarative** 🟠
...
- [ ] **Task 2.3: Rewrite `scroll-to-top` / `smooth-scroll` feedback to declarative** 🟠
...
- [ ] **Task 2.4: Move carousel init-time a11y attributes to server render** 🟠
...
- [ ] **Task 2.5: Move `openModalCount` into store state** 🟡
...
## Phase 3 — Build & enqueue modernization
...
- [ ] **Task 4.1: Stop mangling theme action keys with `sanitize_key`** 🟡
...
- [ ] **Task 4.2: Replace `wp_kses_post()` on context values with scalar validation** 🟡
...
- [ ] **Task 4.3: Drop or scope rate limiting** 🟡
...
- [ ] **Task 5.6: Add `BLOCK_ACTIONS_VERSION` + `BLOCK_ACTIONS_DIR` constants** 🟢
...
- [ ] **Task 5.7: Update CLAUDE.md + memory** 🟢
